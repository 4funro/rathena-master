...
static int is_attack_piercing(struct Damage wd, struct block_list *src, struct block_list *target, int skill_id, int skill_lv, short weapon_position)
{
	...
	if(src != NULL) {
		...
		if( skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS
			&& skill_id != PA_SHIELDCHAIN && skill_id != KO_HAPPOKUNAI ) // Renewal: Soul Breaker no longer gains ice pick effect and ice pick effect gets crit benefit [helvetica]
#else
		if( skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS && skill_id != NPC_GRANDDARKNESS
	...
	switch(skill_id) {
		...
		case TK_JUMPKICK:
			...
			if (sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == skill_id)
				...
				skillratio += -100 + 40 * status_get_lv(src); //Running formula [4%*baselevel]
				if (sc && sc->data[SC_SPURT]) //Spurt formula [8%*baselevel]
					skillratio *= 2;
			}
			else
				skillratio += -70 + 100* skill_lv;
			break;
		...
		case NC_BOOSTKNUCKLE:
			skillratio += 130 + 100 * skill_lv + status_get_dex(src);
			RE_LVL_DMOD(120);
			break;
		case NC_PILEBUNKER:
			skillratio += 500 + 100 * skill_lv + status_get_str(src);
			RE_LVL_DMOD(100);
			break;
		case NC_VULCANARM:
		...
		case NC_MAGMA_ERUPTION: // 'Slam' damage
			...
			break;
		case NC_AXETORNADO:
			skillratio += 300 + 100 * skill_lv + status_get_vit(src);
			RE_LVL_DMOD(100);
		...
		case SC_TRIANGLESHOT:
			skillratio += 700 + (skill_lv - 1) * status_get_agi(src) / 2;
			RE_LVL_DMOD(120);
			break;
		case SC_FEINTBOMB:
			skillratio += -300 + (skill_lv + 1) * status_get_dex(src)  * ((sd) ? sd->status.job_level / 10 : 1);
			RE_LVL_DMOD(120);
			break;
		case LG_CANNONSPEAR:
		...
		case WM_REVERBERATION_MELEE:
			...
			skillratio += 400 + 100 * ((sd) ? pc_checkskill(sd, WM_REVERBERATION) : 1);
			RE_LVL_DMOD(100);
			break;
		...
		case GN_SPORE_EXPLOSION:
			skillratio += 150 + status_get_int(src) + 100 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case GN_WALLOFTHORN:
			skillratio += 10 * skill_lv;
		...
		case SO_VARETYR_SPEAR://ATK [{( Striking Level x 50 ) + ( Varetyr Spear Skill Level x 50 )} x Caster Base Level / 100 ] %
			skillratio += -100 + 80 * skill_lv + ((sd) ? pc_checkskill(sd, SO_STRIKING) * 50 : 0);
			skillratio += -100 + 80 * skill_lv + ((sd) ? pc_checkskill(sd, SO_STRIKING) * 50 : 0);
			RE_LVL_DMOD(100);
...
struct Damage battle_calc_defense_reduction(struct Damage wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	ATK_ADD2(wd.damage, wd.damage2,
		is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ? ((def1+vit_def*3)*battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv))/15 : 0,
		is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ? ((def1+vit_def*3)*battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv))/15 : 0
	);
	...
	if( is_attack_left_handed(src, skill_id) && !attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L) && !is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) )
		...
		ATK_RATE2(wd.damage, wd.damage2,
			attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ?100:(is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)*(def1+vit_def) : (50-def1)),
			attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ?100:(is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L)*(def1+vit_def) : (50-def1))
		);
		ATK_ADD2(wd.damage, wd.damage2,
...
#define MATK_ADD(a) { ad.damage += a; }
				...
				switch(skill_id) {
					...
					case MG_LIGHTNINGBOLT:
						if (sc && sc->data[SC_SPELLFIST] && mflag&BF_SHORT)  {
							skillratio += (sc->data[SC_SPELLFIST]->val4 * 100) + (sc->data[SC_SPELLFIST]->val1 * 100) - 70;// val4 = used bolt level, val2 = used spellfist level. [Rytech]
					...
					case AL_HOLYLIGHT:
						...
						if (sd && sd->sc.data[SC_SPIRIT] && sd->sc.data[SC_SPIRIT]->val2 == SL_PRIEST)
							skillratio *= 20; //Does 5x damage include bonuses from other skills?
						break;
					case AL_RUWACH:
						skillratio += 45;
						break;
					...
					case SO_EARTHGRAVE:
						...
						break;
					case SO_DIAMONDDUST:
						skillratio = ( 300 * ((sd) ? pc_checkskill(sd, SA_FROSTWEAPON) : 0) + sstatus->int_ * skill_lv );
						RE_LVL_DMOD(100);
					...
					case SO_POISON_BUSTER:
						...
						break;
					case SO_PSYCHIC_WAVE:
						skillratio += -100 + 80 * skill_lv + 3 * sstatus->int_;
						RE_LVL_DMOD(100);
		...
		if (skill_id == NPC_EARTHQUAKE) {
			...
			if(mflag>0)
				ad.damage+= (sstatus->rhw.atk2*skillratio/80)/mflag;
			else
